 ...
 *
 *	William Bader, 1990, williambader@hotmail.com http://williambader.com/
 *	- added emulation for DEC EDT keypad commands on VT100 terminals
 */
...
NOSHARE int termflag;		/* f: bit 0: (1) keep terminal in normal mode */
				/* s: bit 1: (2) disable vt100 scrolling */
				/* r: bit 2: (4) disable reverse video */
...
#if	CALLABLE
NOSHARE int exitflag;		/* set when time to exit */

/* free all dynamically allocated areas */

static VOID freeall PP((void));
...
{
	int status;
	status = emacspgm(argc, argv);
	freeall();
	return(status);
}

/* edit from one long command line */
/*   returns 0 if OK */
...
char *name;
{
	...
	return emacsmain(argc, argv);
}

int emacspgm(argc, argv)
#else
int main(argc, argv)
#endif
...
#define	MAX_GLINE 2
        int gline[ MAX_GLINE ];         /* if so, what line? */
        int searchflag;                 /* Do we need to search at start? */
	int savelflag;			/* temp store for lastflag */
	int splitflag;			/* Flag to split window */
	char *parg;			/* Used for scanning arguments */
	...
	for (carg = 1; carg < argc; ++carg) {
		...
		if (argv[carg][0] == '-') {
			switch (argv[carg][1]) {
				...
				case 'G':
					if (gotoflag < MAX_GLINE) {
						gline[ gotoflag++ ] = asc_int(&argv[carg][2]);
					}
					break;
				case 'i':	/* -i<var> <value> */
			...
			bytecopy(bp->b_fname, argv[carg], NFILEN-1);
			bp->b_active = FALSE;
			if (firstfile) {
				firstbp = bp;
				firstfile = FALSE;
			...
			if (cryptflg) {
				...
				me_crypt(ekey, (unsigned) strlen(ekey));
	...
	if (menuflag) menuinit();
        ...
        /* Deal with second window */
	if (splitflag) {
		splitwind(FALSE, 1);
		nextbuffer(FALSE, 1);
		if (gotoflag > 1) {
			/* go to a line */
			/* no use checking the result because any message will be overwritten */
			gotoline(TRUE,gline[1]);
		}
		nextwind(FALSE, 1);
	}
		...
		if (gotoline(TRUE,gline[0]) == FALSE) {
                	update(FALSE);
...
static int useoldargs = FALSE;	/* saved universal arg so C-U n meta command */
static int execnold;		/* repeats command instead of meta */
static int execfold;
NOSHARE int inmeta = FALSE;   /* true if next command was prefixed by a meta */
NOSHARE int incex = FALSE;    /* true if next command was prefixed by a C-X */
NOSHARE int inspecial = FALSE;/* true if next command was prefixed by a FN */

int meta(f, n)	/* dummy function for binding to meta prefix */
int f, n;
{
	inmeta = TRUE;
	useoldargs = TRUE;
...
int cex(f, n)	/* dummy function for binding to control-x prefix */
int f, n;
{
	incex = TRUE;
	useoldargs = TRUE;
...
#if	DECEDT
int special(f, n)	/* dummy function for binding to special prefix */
int f, n;
{
	inspecial = TRUE;
	useoldargs = TRUE;
...
/* If f is TRUE, print a NL after each save message and repaint screen */
...
#endif
	if ((status = quicksave(TRUE, 1)) != TRUE)
		return(status);
