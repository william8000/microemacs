...
static int ldelnewline PP((void));

NOSHARE extern int didldel, didlins;
...
int n, c;
{
	...
	while (wp != NULL) {
		if (wp->w_linep == lp1_save)
			wp->w_linep = lp2;
		if (wp->w_dotp == lp1_save) {
			wp->w_dotp = lp2;
			if (wp==curwp || wp->w_doto>doto)
				wp->w_doto += n;
		}
		if (wp->w_markp == lp1_save) {
			wp->w_markp = lp2;
			if (wp->w_marko > doto)
				wp->w_marko += n;
		}
...
CONSTA char *iline;	/* contents of new line */
